import type { TrackStructureNode } from '@/domain/blueprint'
...
export function ChordsTab({ structure, onExport }: ChordsTabProps) {
  if (!structure?.length) {
    ...
    <div className="flex flex-col gap-8 max-w-4xl">
      <div className="flex justify-between items-center mb-4">
        ...
        <button
          ...
          onClick={onExport}
        >
          <Download className="h-4 w-4" /> Export Chord Sheet
        </button>
      </div>
        ...
        {structure.map((section) => (
          <div key={section.id} className="flex flex-col gap-3 rounded-xl bg-[var(--riff-surface-low)] border border-[var(--riff-surface-highest)] p-6 shadow-sm">
             <div className="flex justify-between items-end border-b border-[var(--riff-surface-lowest)] pb-3">
               ...
               <span className="text-xs font-mono text-[var(--riff-text-muted)]">
                 {formatTimestamp(section.startTime)} · {section.duration}s
               </span>
             </div>
             <div className="flex flex-wrap gap-4 mt-2">
               {section.chords.length ? section.chords.map((chord, i) => (
                 ...
                 <div className="rounded-lg border border-dashed border-[var(--riff-surface-highest)] px-4 py-3 text-sm text-[var(--riff-text-muted)]">
                   No chord map available for this section yet.
                 </div>
               )}
             </div>
          </div>
